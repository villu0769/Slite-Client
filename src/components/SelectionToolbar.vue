<template>
  ...
  >
    ...
    <div v-if="objType!=='window' && objType!=='door' && objType!=='ceiling' && objType!=='floor' && objType!=='wall'"  class="divider"></div>

    <button v-if="objType!=='window' && objType!=='door' && objType!=='ceiling'" class="toolbar-btn" @click="$emit('rotate', -90)" title="Rotate Left">
    ...
    <button v-if="objType!=='window' && objType!=='door' && objType!=='ceiling' " class="toolbar-btn" @click="$emit('rotate', 90)" title="Rotate Right">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <button v-if="objType!=='window' && objType!=='door' && objType!=='ceiling' && objType!=='floor' && objType!=='wall'"  class="toolbar-btn" @click="$emit('flip', 'x')" title="Flip Horizontal">
    ...
    <button v-if="objType!=='window' && objType!=='door' && objType!=='ceiling' && objType!=='floor' && objType!=='wall'"  class="toolbar-btn" @click="$emit('flip', 'z')" title="Flip Vertical" style="transform: rotate(90deg)">
       <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.59 18.512l1.41-1.415L23.113 12l-4.113-5.097-1.41 1.415L20.285 12l-2.695 6.512zM12 2v20M6.41 18.512l-1.41-1.415L.887 12l4.113-5.097 1.41 1.415L3.715 12l2.695 6.512z"/></svg>
    </button>
